  ...
  <div class="app-container">
    ...
    <el-card class="box-card">
      ...
      <el-form ref="goods" :rules="rules" :model="goods" label-width="150px">
        ...
        <el-form-item label="专柜价格" prop="counterPrice">
          <el-input v-model="goods.counterPrice" onkeyup="this.value=this.value.replace(/[^\d.]/g,'');" placeholder="0.00">
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        <el-form-item label="当前价格" prop="retailPrice">
          <el-input v-model="goods.retailPrice" onkeyup="this.value=this.value.replace(/[^\d.]/g,'');" placeholder="0.00">
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        <!--<el-form-item label="佣金设置" prop="brokerageType">-->
          <!--<el-radio-group v-model="goods.brokerageType" @change="brokerageTypeHandler">-->
            <!--<el-radio :label="0">无佣金</el-radio>-->
            <!--<el-radio :label="1">自定义</el-radio>-->
          <!--</el-radio-group>-->
        <!--</el-form-item>-->
        <!--<el-form-item v-show="brokerageVisible" label="推广佣金" prop="brokeragePrice">-->
          <!--<el-input v-model="goods.brokeragePrice" onkeyup="this.value=this.value.replace(/[^\d.]/g,'');" placeholder="0.00">-->
            <!--<template slot="append">元</template>-->
          <!--</el-input>-->
        <!--</el-form-item>-->
    ...
    <el-card class="box-card">
      ...
      <el-dialog :visible.sync="productVisiable" title="设置货品">
        <el-form ref="productForm" :rules="rules" :model="productForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
...
export default {
  ...
  data() {
    return {
      ...
      brandList: [],
      categoryIds: [],
      goods: { gallery: [], brokerageType: 0, brokeragePrice: 0 },
      ...
      rules: {
        ...
        brandId: [{ required: true, message: '品牌商不能为空', trigger: 'blur' }],
        retailPrice: [{ required: true, message: '商品当前价格不能为空', trigger: 'blur' }],
        price: [{ required: true, message: '货品售价不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    brokerageTypeHandler(value) {
      this.goods.brokerageType = value
      if (value === 1) {
        this.brokerageVisible = true
      } else {
        this.goods.brokeragePrice = 0
        this.brokerageVisible = false
      }
    },
    ...
    handleEdit: function() {
      this.$refs['goods'].validate(valid => {
        if (valid) {
          if (this.goods.brokerageType === 1) {
            if (this.goods.brokeragePrice < 0.01) {
              this.$notify.error({
                title: '温馨提示',
                message: '请输入推广佣金金额'
              })
            ...
            if (this.goods.brokeragePrice * 2 > this.goods.retailPrice) {
              this.$notify.error({
                title: '温馨提示',
                message: '推广佣金金额不能超商品价格的50%'
              })
              return
            }
          }
