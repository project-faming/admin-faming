<template>
	<div class="app-container">
		...
		<div class="filter-container">
			<el-input v-model="listQuery.id" clearable size="mini" class="filter-item" style="width: 200px;" placeholder="请输入门店ID"/>
		...
		<el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
			...
			<el-table-column align="center" label="操作" min-width="200" class-name="small-padding fixed-width">
				<template slot-scope="scope">
					<!--<el-button type="primary" size="mini" @click="shareUrlDetail(scope.row)">门店码</el-button>-->
		...
		<el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
			<el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
				...
				<!--<el-form-item label="提货时间设置">-->
				<!--<el-tag v-for="tag in dataForm.fetchTimeRules" :key="tag" closable type="primary" @close="handleClose(tag)">-->
				<!--{{ tag }}-->
				<!--</el-tag>-->
				<!--<el-time-picker-->
				<!--v-if="newTimeRulesVisible"-->
				<!--ref="newTimeRulesInput"-->
				<!--v-model="newTimeRules"-->
				<!--format="HH:mm"-->
				<!--value-format="HH:mm"-->
				<!--class="input-new-time-rules"-->
				<!--size="small"-->
				<!--is-range-->
				<!--range-separator="至"-->
				...
				<!--<el-button v-else class="button-new-time-rules" size="small" type="primary" @click="addTimeRules">+ 增加</el-button>-->
				<!--</el-form-item>-->
			...
			<el-form :inline="true" :model="dataForm" status-icon label-position="left" label-width="100px" style="margin-left:50px;">
				<el-form-item label="经纬度" prop="longitude">
					<el-input v-model="dataForm.longitude" style="width: 120px;" placeholder="输入经度值"/>
				</el-form-item>
				<el-form-item label="" prop="latitude">
					<el-input v-model="dataForm.latitude" style="width: 120px;" placeholder="输入纬度值"/>
				</el-form-item>
				<el-form-item>
					<el-button type="primary" @click="goLbs">获取经纬度</el-button>
				</el-form-item>
			</el-form>
    ...
    export default {
        ...
        methods: {
            ...
            handleUpdate(row) {
                ...
                this.fetchTimeRules = this.dataForm.fetchTimeRules ? this.dataForm.fetchTimeRules : []
            ...
            addTimeRules() {
                this.newTimeRulesVisible = true
            },
            goLbs() {
                const href = 'https://lbs.qq.com/tool/getpoint/index.html'
                window.open(href, '_blank', 'toolbar=yes, width=1300, height=900')
            },
            handleClose(tag) {
                this.fetchTimeRules.splice(this.fetchTimeRules.indexOf(tag), 1)
                this.dataForm.fetchTimeRules = this.fetchTimeRules
            },
            handleInputConfirm() {
                const newTimeRules = this.newTimeRules
                if (newTimeRules && newTimeRules.length > 1) {
                    this.fetchTimeRules.push(newTimeRules[0] + ' - ' + newTimeRules[1])
                    this.dataForm.fetchTimeRules = this.fetchTimeRules
                }
                this.newTimeRulesVisible = false
                this.newTimeRules = ''
            }
        }
    }
